...
function Berry({berryName}) {
  
  
  const [isLoading, setIsLoading] = useState(true);
  const [berry, setBerry] = useState(berryName);
  ...
  if (isLoading) {
    ...
    return (
      ...
      <img className="w-24 h-24 mb-3 rounded-full shadow-lg" src={berry.sprites.default} alt={berry.name}/>
      ...
      <h3 className='inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white rounded-lg bg-blue-300'>{berry.category.name}</h3>
      </div>
      </div>
